...
def sort_events(events):
    ...
    return sorted(
        events,
        key=lambda x: make_timezone_aware(dateutil.parser.parse(x["calculated"]["startDate"]))
    )
...
events = expanded_data["data"]

# Sort the events by date
sorted_data = sort_events(events)

print(f"[2_sort_events.py] Sorted {len(sorted_data)} events")

# Save the sorted events back to a JSON file
